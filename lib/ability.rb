...
class Ability
  ...
  def configure_viewer(user)
    ...
    can :read, Template, scope
    can :read, TemplateFolder, scope
    can :read, TemplateSharing, template: scope
    can :read, Submission, scope
    can :read, Submitter, scope
  ...
  def configure_editor(user)
    ...
    can %i[read create update], Template, scope
